...
class MonteCarloAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        # raise NotImplementedError("Needed for Q2")
        G = 0
        episode = list(zip(obses, actions))
        for i in range(len(episode) -1, -1, -1):
            G = self.gamma * G + rewards[i]

            ep = episode[i]
            
            if ep not in self.sa_counts.keys():
                self.sa_counts[ep] = 0
            
            self.q_table[ep] = self.q_table[ep] + ((G - self.q_table[ep])/(self.sa_counts[ep] + 1))
            updated_values[ep] = self.q_table[ep]
            self.sa_counts[ep] += 1
