...
if __name__ == "__main__":
    ...
    random_seeds = [
    42, 123, 999, 2025, 7, 88, 313, 77, 56, 908,
    65432, 2718, 314159, 8675309, 13579
    ]
    runs_list = []
    Flag = True #change this flag when needed is_slippery
    for seed in random_seeds:
        config = CONFIG.copy()
        config["save_filename"] = f"qlearning_seed_{seed}_flag_{Flag}" 
        config["seed"] = seed
        run = Run(config)
        run.run_name = f"q_learning_{seed}_flag_{Flag}"
        env = gym.make(CONFIG["env"], is_slippery=Flag)
    ...
    for i, r in enumerate(ranked_runs):
        ...
        average += r.final_return_mean
    print(f"Avergae Return = {average/len(random_seeds)}")
    print("Done!")
