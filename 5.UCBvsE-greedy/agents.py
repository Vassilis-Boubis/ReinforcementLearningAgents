...
from rl2025.exercise5.networks import FCNetwork
from rl2025.exercise5.replay import Transition

class Agent(ABC):
    """Base class for Deep RL Exercise 5 Agents
    ...
    **DO NOT CHANGE THIS CLASS**
    ...
    @abstractmethod
    def act(self, obs: np.ndarray, timestep: bool):
        ...
    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.epsilon_t = epsilon_start

        self.n_acts: int = action_space.n

        self.c = c_value
        self.action_counts: DefaultDict = defaultdict(lambda: 0)

        # self.exploration_strategy = "ucb"
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(timestep, max_timestep, epsilon_start, epsilon_min, exploration_fraction):
            ...
            if timestep / max_timestep >= self.exploration_fraction:
                ...
                decay_rate = (self.epsilon_start - self.epsilon_min) / decay_steps
                return self.epsilon_start - decay_rate * timestep
        ...
        def epsilon_exponential_decay(timestep, epsilon_start, epsilon_min, decay_factor):
            ...
            ### PUT YOUR CODE HERE ###
            temp = self.epsilon_t
            self.epsilon_t = (self.epsilon_exponential_decay_factor ** (timestep / max_timestep)) * temp
            return max(self.epsilon_min, self.epsilon_t)
    ...
    def act(self, obs: np.ndarray, explore: bool, timestep: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q5**
        ...
        q_values = self.critics_net(obs_tensor).detach().numpy()
        ...
        if self.exploration_strategy == "epsilon-greedy":
            ...
            for a in range(self.n_acts):
                key = (state_key, a)
                visits = self.action_counts[key] + 1
                bonus = self.c * np.sqrt(np.log(timestep)/ visits)
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q5**
        ...
        # Calculate current Q-values
         ### PUT YOUR CODE HERE ###
        q = self.critics_net(states).gather(1, actions)
        ...
        with torch.no_grad():
            ...
            ### PUT YOUR CODE HERE ###
            expected_q = rewards + self.gamma * (1 - dones) * max_q
        
        # Calculate loss (Mean Squared Error)
        ### PUT YOUR CODE HERE ###
        loss_fn = torch.nn.MSELoss()
        q_loss = loss_fn(q, expected_q)
...
class DiscreteRL(Agent):
    """The DiscreteRL Agent for Ex 5 using tabular Q-Learning without neural networks
    ...
    def act(self, obs: np.ndarray, timestep: bool, explore: bool = True) -> int:
        ...
        if self.exploration_strategy == "epsilon-greedy":
            ...
            for a in range(self.n_acts):
                ...
                count = self.action_counts[(state, a)] + 1
                bonus = self.c * np.sqrt(np.log(timestep+1) / (count))
    ...
    def update(
         ...
         ** YOU NEED TO IMPLEMENT THIS FUNCTION FOR Q5 BUT YOU CAN REUSE YOUR Q LEARNING CODE FROM Q2 (you can include it here or you adapt the files from Q2 to work of the mountain car problem **
        ...
        ### PUT YOUR CODE HERE ###
        q_next = np.argmax([self.q_table[(next_state, a)] for a in range(self.n_acts)])

        self.q_table[(state, action)] = self.q_table[(state, action)] + self.alpha * (
            reward + self.gamma * (1 - done) * self.q_table[(next_state, q_next)] - self.q_table[(state, action)]
        )
