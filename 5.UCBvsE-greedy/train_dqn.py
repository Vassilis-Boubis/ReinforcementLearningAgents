...
MOUNTAINCAR_HPARAMS_EXP_DECAY = {
    "epsilon_start": [1.0, ],
    "epsilon_decay": [1.0, 0.5, 1e-05]
    }

MOUNTAINCAR_HPARAMS_CONS = {
    # "learning_rate": [3e-4],
    "c_value": [0.5]
    }
...
if MOUNTAINCAR_CONFIG['epsilon_decay_strategy'] == "linear":
    ...
    MOUNTAINCAR_HPARAMS = MOUNTAINCAR_HPARAMS_CONS
...
def play_episode(
    ...
    while not done:
        action = agent.act(obs, timestep=episode_timesteps, explore=explore)
...
if __name__ == "__main__":
    ...
    if SWEEP and HPARAMS_SWEEP is not None:
        ...
        # np.savetxt("eval_returns_dqn_epsilon_greedy.csv", eval_returns, delimiter=",", fmt="%.4f") 
