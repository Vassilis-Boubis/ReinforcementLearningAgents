...
from rl2025.exercise5.agents import DiscreteRL
...
if MOUNTAINCAR_CONFIG['epsilon_decay_strategy'] == "linear":
    ...
    MOUNTAINCAR_HPARAMS = MOUNTAINCAR_HPARAMS_CONST
...
def play_episode(
    ...
    while not done and num_steps < max_steps:
        action = agent.act(np.array(obs), timestep=num_steps, explore=explore)
...
if __name__ == "__main__":
    ...
    if SWEEP:
        ...
        # np.savetxt("eval_returns_discrete_epsilon_greedy.csv", eval_returns, delimiter=",", fmt="%.4f") 
    env.close()
