...
class DDPG(Agent):
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        obs_torch = torch.from_numpy(np.array(obs)).float()
        action = self.actor(obs_torch).detach().numpy()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        with torch.no_grad():
            ...
            q_pred = rewards + (self.gamma * (1 - done) * next_q)

        current_q = self.critic(torch.cat((states, actions), dim=1))

    
        loss = torch.nn.MSELoss()
        q_loss = loss(q_pred, current_q)
        ...
        p_loss = - self.critic(torch.cat((states, self.actor(states)), dim=1))
        ...
        # self.critic_target.parameters() = ((1 - self.critic_learning_rate) * self.critic_target.parameters()) + (self.critic_learning_rate * self.critic_target.parameters())
        # self.actor_target.parameters() = ((1 - self.policy_learning_rate) * self.actor_target.parameters()) + (self.policy_learning_rate * self.actor_target.parameters())
