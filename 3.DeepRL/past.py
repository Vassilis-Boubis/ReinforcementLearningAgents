    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_exponential_decay(*args, **kwargs):
            ### PUT YOUR CODE HERE ###
            # raise(NotImplementedError)
            epsilon_t = (self.epsilon_exponential_decay_factor ** (timestep / max_timestep)) * self.epsilon_start
            return max(self.epsilon_min, epsilon_t)
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        max_q = self.critics_target(nobs).max(axis=1)[0].detach().unsqueeze(1)
        expected_q = reward + self.gamma * (1 - done) * max_q
        q = self.critics_net(obs).gather(1, action.long())
...
class DiscreteRL(Agent):
    ...
    def learn(
        ...
        ** YOU NEED TO IMPLEMENT THIS FUNCTION FOR Q3 BUT YOU CAN REUSE YOUR Q LEARNING CODE FROM Q2 **
        ...
        q_next = np.argmax([self.q_table[(n_obs, a)] for a in range(self.n_acts)])

        self.q_table[(obs, action)] = self.q_table[(obs, action)] + self.alpha * (
            reward + self.gamma * (1 - done) * self.q_table[(n_obs, q_next)] - self.q_table[(obs, action)]
        )
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS BASED ON YOUR WORK FOR EX 2**
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        log = self.policy(obs)
        dist = Categorical(log)
        probabilities = dist.log_prob(torch.tensor(actions))

        G = 0
        for i in range(len(rewards) -1, -1, -1):
            G = self.gamma * G + rewards[i]
            p_loss = p_loss - (probabilities[i] * G)
